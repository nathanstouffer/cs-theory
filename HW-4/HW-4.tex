...
Then $L(M_1)$ will always contain at least all strings of odd length. However, $L(M_1)$ will also contain strings of even length if $M$ accepts $w$. That is, the membership of $M_1$ in $ODD_{TM}$ depends entirely on whether $M$ accepts $w$. We will now construct a $TM$ to decide $A_{TM}$: \parspace
